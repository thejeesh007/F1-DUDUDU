...
DATA_DIR = "f1_data"
CACHE_DIR = "f1_cache"
SEASONS = list(range(2018, 2027))  # 2018-2026, matches our other data
...
def main():
    ...
    for season in SEASONS:
        ...
        for _, event in race_rounds.iterrows():
            ...
            # Skip races that haven't happened yet WITHOUT burning through
            # retries - FastF1 would otherwise take ~35s (3 retries with
            # backoff) to fail on a race that simply hasn't run, and this
            # script is meant to be rerun every race weekend, so wasting
            # that time on every future race, every single run, adds up.
            event_date = event.get("EventDate")
            if pd.notna(event_date) and pd.Timestamp(event_date) > pd.Timestamp.now():
                print(f"  Round {round_num}: {race_name} ... not yet run, skipping")
                continue
